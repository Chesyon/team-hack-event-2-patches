...
int __attribute__((used)) TryHandleMoveShortcuts(struct entity *leader) {
    ...
    do {
        ...
        // Check that only ONE shortcut has been used. If the input is ambigious do nothing.
        bool ambiguity_check = (a_button && b_button) || (a_button && x_button) || (a_button && y_button);
        ambiguity_check |= (b_button && x_button) || (b_button && y_button);
        ambiguity_check |= x_button && y_button;
        if (!ambiguity_check) { // If the move input is NOT ambigious.
            int move_to_use_index = -1;
            if (a_button) {
                move_to_use_index = 0;
            } else if (b_button) {
                move_to_use_index = 1;
